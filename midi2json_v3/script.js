"use strict";
const _i = ['MIDI转JSON', [3, 0], 1585107102, 1612073331];
const out = document.getElementById("output");
const div = document.getElementById("track");
/*result start*/
const data = {};
const tempo = [];
const tracks = [];
...
function analyse() {
	...
	reader.onload = function() {
		...
		for (const i in tracks) {
			...
			option.innerHTML = single ? "Track" : i;
			option.value = i;
			document.getElementById("track").appendChild(option);
		}
		if (drum.length) {
			let option = document.createElement("option");
		...
		out.className = "accept";
		out.innerHTML = `分析成功(${(end - start) / 1000}s)：可转换${tracks.length+!!drum.length}条音轨<br>Duration:&nbsp;${Math.round(data.dur*1000)/1000}s&emsp;BPM:&nbsp;${Math.round(data.bpm*1000)/1000}`;
		/*test1*/
...
function convert() {
	...
	const start = new Date().getTime();
	const minnote = document.getElementById("note").value;
	const tracksss = div.value;
	test = (tracksss > -1) ? tracks[tracksss] : drum;
	var bpm = data.bpm; //以后可能添加自定义bpm
	var minp = Math.round(data.mindur * bpm / data.time / 6e7 * minnote) * 32 / minnote;
	var maxp = Math.round(data.maxdur * bpm / data.time / 6e7 * minnote) * 32 / minnote;
	var result = []; //储存结果
	var notes = []; //储存多重音符
	var dur = 0; //储存延时
	var flag = 0; //储存剩余休止符个数
	for (const i in test) {
		test[i].p = Math.round(test[i].duration * bpm / data.time / 6e7 * minnote) * 32 / minnote; //[K]/[P]=32
		if (!test[i - 1]) { //单独考虑第一个音符
			dur += test[i].p - minp;
			if (dur > 0)
				result.push({
					notes: [128],
			...
			dur = 0;
			if (test[i].type) {
				notes.push(test[i].note);
				flag++;
			}
		} else { //考虑非第一个音符
			dur += test[i].p - test[i - 1].p; //前后时间差
			if (test[i].type) { //on事件
				if (dur) { //不为零，则前一音符事件结束
					result.push({
						notes: unique(notes),
						p: dur
					});
				...
				notes.push(test[i].note); //加入事件
				flag++;
			} else if (flag > 0)
				flag--; //off事件
			else if (dur && notes.length) { //flag为零，前一音符事件结束
				result.push({
					notes: unique(notes),
					p: dur
				});
	...
	dur = maxp - test[test.length - 1].p;
	if (dur > 0) result.push({
		notes: [128],
		p: dur
	});
	/*test1-2*/
	...
	result.forEach((item, i, arr) => {
		if (item.notes.length > 1) {
			const notes = [];
			for (const j of item.notes)
				notes.push(pt2Note[j]);
			text += `(${notes.join(".")})[${numToLength(item.p, 1)}],`;
		} else if (item.notes[0] != 128) text += `${pt2Note[item.notes[0]]}[${numToLength(item.p, 1)}],`;
		else if (arr[i + 1] && arr[i + 1].notes[0] == 128) arr[i + 1].p += item.p;
		else text += `${numToLength(item.p, 0)},`;
	});
...
function numToLength(num, type) { //type:1-音符,0-休止符
	let text = "";
	while (num >= 256) {
		text += type ? "H" : "Q";
		num -= 256;
